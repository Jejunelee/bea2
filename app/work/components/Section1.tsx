...
// Sample data - REPLACE WITH ACTUAL POST URLs
...
const spotifyPodcasts: SpotifyPodcast[] = [
  {
    embedUrl:
      "https://open.spotify.com/embed/episode/5s5ZBx5isGUg2kActdsuln?utm_source=generator",
    title: "",
    platform: "spotify",
  },
  {
    embedUrl:
      "https://open.spotify.com/embed/episode/2lmJJIIVlRcva6CiW9Z2tJ?si=61155a1a76694263",
    title: "",
    platform: "spotify",
  },
  {
    embedUrl:
      "https://open.spotify.com/embed/episode/7gg812xfQA9gMKiB7XieOJ?si=07973a2e1e68461b",
    title: "",
    platform: "spotify",
  },
];
...
// Loading placeholder for embeds
...
}> = ({ title, icon, iconBg, platform, count, visible, onToggle, children }) => {
  return (
    <div className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 transition-all duration-300">
      {/* Header - Clickable to toggle */}
      <button
        onClick={onToggle}
        className="w-full text-left transition-all duration-200 hover:opacity-90"
      >
        <div className={`${iconBg} p-4`}>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              {platform === 'linkedin' && (
                <a
                  href="https://www.linkedin.com/in/beatrinidad/"
                  target="_blank"
                  rel="noopener noreferrer"
                  onClick={(e) => e.stopPropagation()}
                  className="text-white/90 hover:text-white text-sm font-medium transition"
              ...
              <i className={`fas fa-chevron-${visible ? 'up' : 'down'} text-white text-xl transition-transform duration-300`}></i>
            </div>
          </div>
        </div>
      </button>
...
const InstagramSection: React.FC<{
  ...
  isExpanded: boolean;
  onToggle: () => void;
}> = ({ posts, searchQuery, isExpanded, onToggle }) => {
  const [mounted, setMounted] = useState(false);
  const filteredPosts = filterItemsBySearch(posts, searchQuery);
    ...
    >
      {filteredPosts.length === 0 ? (
        <div className="text-center py-16 bg-gray-50 rounded-xl">
          <i className="fab fa-instagram text-4xl text-gray-300 mb-3 block"></i>
          <p className="text-gray-500">No Instagram posts match "{searchQuery}"</p>
        </div>
      ) : (
...
const TikTokSection: React.FC<{
  ...
  return (
    ...
    >
      {filteredPosts.length === 0 ? (
        <div className="text-center py-16 bg-gray-50 rounded-xl">
          <i className="fab fa-tiktok text-4xl text-gray-300 mb-3 block"></i>
          <p className="text-gray-500">No TikTok videos match "{searchQuery}"</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredPosts.map((post, idx) => (
...
const LinkedInSection: React.FC<{
  ...
  return (
    ...
    >
      {filteredPosts.length === 0 ? (
        <div className="text-center py-16 bg-gray-50 rounded-xl">
          <i className="fab fa-linkedin-in text-4xl text-gray-300 mb-3 block"></i>
          <p className="text-gray-500">No LinkedIn posts match "{searchQuery}"</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredPosts.map((post, idx) => (
              ...
              <div className="p-4">
                <div className="bg-gray-50 rounded-lg overflow-hidden">
                  {mounted ? (
                    ...
                    <div className="w-full h-[350px] bg-gray-100 animate-pulse rounded-lg flex items-center justify-center">
                      <div className="text-gray-400">Loading content...</div>
                    </div>
                  )}
                </div>
...
// Spotify Podcasts Grid Section
const SpotifySection: React.FC<{
  podcasts: SpotifyPodcast[];
  searchQuery: string;
  isExpanded: boolean;
  onToggle: () => void;
}> = ({ podcasts, searchQuery, isExpanded, onToggle }) => {
  const filteredPodcasts = filterItemsBySearch(podcasts, searchQuery);
  ...
  return (
    ...
    >
      {filteredPodcasts.length === 0 ? (
        <div className="text-center py-16 bg-gray-50 rounded-xl">
          <i className="fab fa-spotify text-4xl text-gray-300 mb-3 block"></i>
          <p className="text-gray-500">No podcasts match "{searchQuery}"</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
...
export default function SocialFeedPage() {
  ...
  useEffect(() => {
    if (activePlatform === "all") {
      // When "All" is selected, keep sections as they are (don't auto-expand)
      // Users can manually expand what they want
    } else {
      setExpandedSections({
  ...
  // Don't render content that depends on client-only libraries until mounted
  if (!mounted) {
    return (
