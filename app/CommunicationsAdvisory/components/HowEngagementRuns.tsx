...
export default function HowEngagementRuns() {
  ...
  const titleEmphasisWords = settings.title_emphasis_words || {
    "A discovery call": "call",
    "A scoped proposal": "proposal",
    "A three-month minimum": "minimum"
  };
        ...
        <div className={`transition-all duration-700 ease-out ${hasAnimated ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
          <div className="flex flex-wrap justify-center gap-8 md:gap-12">
            {displaySteps.map((step, idx) => (
              <div 
                key={step.id} 
                className={`flex-1 min-w-[200px] max-w-sm text-center transition-all duration-700 ease-out ${hasAnimated ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`} 
                style={{ transitionDelay: `${idx * 150}ms` }}
              >
                <div 
