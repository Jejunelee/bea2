// app/components/foundation/CTASection.tsx
"use client";

import { useEffect, useRef, useState } from "react";
import { supabase } from "@/app/lib/supabase/client";
import type { FoundationCTASettings } from "@/app/types/StoryFoundation/CTASection";

export default function CTASection() {
  const [settings, setSettings] = useState<Partial<FoundationCTASettings>>({});
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      const { data } = await supabase.from('foundation_cta_settings').select('*').eq('id', 1).single();
  ...
  const headline = settings.headline || "Start the foundation";
  const subheadline = settings.subheadline || "If your brand has a sharper story in it and you are ready to do the messaging work properly, the Foundation is how you get there in four to six weeks.";
  const buttonText = settings.button_text || "Book a call";

  const headlineParts = headline.split("foundation");
  const hasFoundation = headlineParts.length > 1;
        ...
        <div className="relative z-10 max-w-3xl mx-auto text-center">
          <div className={`transition-all duration-700 ease-out ${hasAnimated ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>
            <h2 className="text-2xl font-medium text-white mb-3 font-helvetica tracking-tight">
              {hasFoundation ? <>{headlineParts[0]}<span className="font-editorial italic">foundation</span>{headlineParts[1]}</> : headline}
            </h2>
            <p className="text-base text-white/60 leading-relaxed mb-8 font-helvetica max-w-md mx-auto">{subheadline}</p>
            <button onClick={createCalendarEvent} className="group relative rounded-full bg-white text-black text-base font-medium px-8 py-3 shadow-md hover:shadow-lg transition-all duration-200 hover:scale-105 active:scale-95 overflow-hidden">
  ...
  return (
    <section ref={sectionRef} className="relative w-full py-32 px-6 overflow-hidden" style={{ backgroundColor: '#000000' }}>
      <div className="absolute inset-0 pointer-events-none">
      ...
      <div className="relative z-10 max-w-3xl mx-auto text-center">
        <div className={`transition-all duration-700 ease-out ${hasAnimated ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
          ...
          <p className="text-lg md:text-xl text-white/60 leading-relaxed mb-10 font-helvetica max-w-2xl mx-auto">{subheadline}</p>
          <button onClick={createCalendarEvent} className="group relative rounded-full bg-white text-black text-base md:text-lg font-medium px-10 py-4 shadow-md hover:shadow-lg transition-all duration-200 hover:scale-105 active:scale-95 overflow-hidden">
            <span className="relative z-10 flex items-center gap-2">
              {buttonText}
