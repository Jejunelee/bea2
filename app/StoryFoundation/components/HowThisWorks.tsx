...
export default function HowThisWorks() {
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      const { data } = await supabase
        .from('foundation_howitworks_settings')
        .select('*')
        .eq('id', 1)
        .single();
  ...
  const paragraphOne = settings.paragraph_one || "The Foundation is not a brand strategy retreat. There is no offsite, no whiteboard exercise, no manifesto. It is a structured writing engagement that produces real, deployed work.";
  const paragraphOneEmphasis = settings.paragraph_one_emphasis || ["not a brand strategy retreat"];
  
  const paragraphTwo = settings.paragraph_two || "It is also not a content factory. I am not going to send you 50 social captions and call it activation. The three priority assets are chosen because they are the ones that earn or lose customers for you, and they are written carefully, in your voice, to last.";
  const paragraphTwoEmphasis = settings.paragraph_two_emphasis || ["not a content factory"];
  
  const calloutText = settings.callout_text || "If what you actually need is one asset rebuilt rather than three, or a higher volume of content over a longer engagement, that is probably Comms Advisory, not the Foundation.";
  const calloutEmphasis = settings.callout_emphasis || ["Comms Advisory"];
