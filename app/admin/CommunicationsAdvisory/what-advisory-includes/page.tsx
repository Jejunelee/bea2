...
export default function AdminAdvisoryIncludesPage() {
  const [settings, setSettings] = useState<Partial<AdvisoryIncludesSettings>>({});
  const [items, setItems] = useState<AdvisoryInclude[]>([]);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [message, setMessage] = useState('');
  const [activeTab, setActiveTab] = useState('settings');
  const [uploading, setUploading] = useState(false);
  const [selectedItem, setSelectedItem] = useState<AdvisoryInclude | null>(null);
  ...
  const updateItem = async (id: number, field: keyof AdvisoryInclude, value: string | number) => {
  ...
  const handleImageUpload = async (file: File) => {
    ...
    const fileName = `advisory-includes-${Date.now()}.${fileExt}`;
  ...
  return (
    <div className="min-h-screen bg-white p-8">
      <div className="max-w-6xl mx-auto">
        <h1 className="text-3xl font-bold mb-8 text-gray-800">What Advisory Includes - Admin (Advisory)</h1>
        ...
        {activeTab === 'settings' && (
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            ...
            <div className="space-y-6">
              ...
              <div className="border-b border-gray-200 pb-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      type="text"
                      value={settings.section_title || 'What Advisory includes'}
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={settings.italic_word || 'includes'}
              ...
              {/* Footer Settings */}
              <div className="border-b border-gray-200 pb-4">
                <h3 className="text-lg font-medium mb-3 text-gray-800">Footer Settings</h3>
                <div className="space-y-3">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full p-2 border border-gray-300 rounded bg-white text-gray-800"
                    />
                    <p className="text-xs text-gray-500 mt-1">Use "Fractional Comms" as the link text placeholder</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium mb-1 text-gray-700">Footer Link URL</label>
                    <input
                      type="text"
                      value={settings.footer_link || '/FractionalCommunication'}
                      onChange={(e) => setSettings({ ...settings, footer_link: e.target.value })}
                      className="w-full p-2 border border-gray-300 rounded bg-white text-gray-800"
                      placeholder="/FractionalCommunication"
                    />
                  </div>
                </div>
              </div>
                  ...
                  <div className="mb-3">
                    <div className="relative w-full aspect-[11/12] max-w-md">
                      <Image
                        ...
                        alt="Advisory includes illustration"
                        fill
        ...
        {activeTab === 'items' && (
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-semibold text-gray-800">Include Items</h2>
                  ...
                  <div className="mb-3">
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        placeholder="e.g., session per month, brief, check"
                        className="flex-1 p-2 border border-gray-300 rounded bg-white text-gray-800"
                      />
                    </div>
            ...
            >
              ...
              <div className="relative z-10 max-w-6xl mx-auto">
                ...
                <div className="mb-8 text-center">
                  ...
                  >
                    {(settings.section_title || 'What Advisory includes').split(settings.italic_word || 'includes')[0]}
                    <span className="font-editorial italic">
                      {settings.italic_word || 'includes'}
                    </span>
                    {(settings.section_title || 'What Advisory includes').split(settings.italic_word || 'includes')[1]}
                  </h2>
                  <div 
                ...
                {/* Footer Preview */}
                <div className="text-center pt-8 border-t" style={{ borderColor: `${settings.text_color}10` }}>
                  <p 
                    className="text-base md:text-lg"
                    style={{ color: settings.muted_text_color || 'rgba(0, 0, 0, 0.5)' }}
                  >
                    {(settings.footer_text || 'If you need someone in the business running the work, I\'d recommend Fractional Comms →').split('Fractional Comms')[0]}
                    <a 
                      href={settings.footer_link || '/FractionalCommunication'} 
                    ...
                    {(settings.footer_text || 'If you need someone in the business running the work, I\'d recommend Fractional Comms →').split('Fractional Comms')[1]}
                  </p>
                </div>
              </div>
            </div>
